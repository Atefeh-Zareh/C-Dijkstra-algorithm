...
class Graph {
    ...
    struct Edge {
        ...
        Edge(int edge_id, std::string data, int source_id, int dest_id) {
...
public:
    ...
    inline bool edge_exist(int id) const {
        auto iterator = edges.find(id);
        bool doesExist = iterator != edges.end();
        return doesExist;
    }

    inline bool dests_exist(int id) const {
    ...
    inline void remove_node(int node_id) {
        if(node_exist(node_id)){
            ...
            //delete from dests:
            bool doesExist = dests_exist(node_id);
            if(doesExist){
                dests.erase(node_id);
            }
            nodes.erase(node_id);
...
//        Path empty;
//        std::vector <Path> allPaths;
//        all_paths (from, to, empty,allPaths);
//        return allPaths;
//    }

//    void all_paths(int from, int to, Path subPath, std::vector <Path> &allPaths) const {
//        std::cout<<"call all_paths for (from: "<< from << ", to: "<< to << ", subPath.size(): " <<
//                 subPath.size() << ", allPaths.size(): " << allPaths.size() <<std::endl;
//
//        std::vector<int> successors = get_successors(from);
//        for (int successor: successors) {
//            Path subPathSuccessor = subPath;
//            if (successor == to) {
//                subPathSuccessor.push_back(to);
//                allPaths.push_back(subPathSuccessor);
//            } else {
//                subPath.push_back(successor);
//                all_paths(successor, to, subPath, allPaths);
//            }
//        }
//    }
//*********************************************************
