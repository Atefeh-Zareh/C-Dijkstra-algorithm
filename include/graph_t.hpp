...
#ifndef GRAPH_GRAPH_T_HPP
#define GRAPH_GRAPH_T_HPP
...
class Graph {
    /* data structures */
    std::map<int, ND> nodes;
    std::map<int, ED> edges;
    std::map<std::pair< int,int>, ED> edges_detailed;
private:
...
//        ED edge(edge_counter, m, source_id, dest_id);
...
//        return edge_counter++;
//    }
    inline int add_edge(ED edge, int source_id, int dest_id) {
        edges.insert(std::make_pair(edge_counter, edge));
        edges_detailed.insert(std::make_pair(std::make_pair(source_id, dest_id), edge));
    ...
    inline ED get_edge_obj(int src, int dest) const {
    ...
    inline ED get_edge(int edge_id) const {
        auto iterator = edges.find(edge_id);
        if (iterator != edges.end()) {
            return iterator->second;
        }
...
//        ND node(node_counter, m);
//        nodes.insert(std::make_pair(node_counter, node));
//        return node_counter++;
//    }
    inline int add_node(ND node) {
        nodes.insert(std::make_pair(node_counter, node));
        return node_counter++;
    }
